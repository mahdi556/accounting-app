// src/app/detail-accounts/[id]/page.js
'use client'
...
export default function DetailAccountDetailPage() {
  ...
  const fetchDetailAccount = async () => {
    try {
      ...
      console.log('🔍 Fetching detail account with ID:', params.id)
      const response = await fetch(`/api/detail-accounts/${params.id}`)
      
      if (!response.ok) {
        const errorData = await response.json()
        throw new Error(errorData.error || 'خطا در دریافت اطلاعات حساب تفصیلی')
      }
      
      const data = await response.json()
      console.log('✅ Received detail account data:', data)
      setAccount(data)
    } catch (error) {
  ...
  const calculateTurnover = () => {
    ...
    let balance
    const accountType = account.subAccount.category.type
    ...
    return (
      <Container>
        <div className="text-center p-5">
          ...
          <p className="mt-3">در حال بارگذاری اطلاعات حساب تفصیلی...</p>
    ...
    return (
      <Container>
        <Alert variant="danger">
          ...
          <p>{error || 'حساب تفصیلی یافت نشد'}</p>
          <p className="text-muted">شناسه درخواستی: {params.id}</p>
          <div className="d-flex gap-2 mt-3">
            <Button variant="outline-danger" onClick={() => router.push('/detail-accounts')}>
              بازگشت به لیست حساب‌های تفصیلی
            </Button>
            <Button variant="outline-primary" onClick={fetchDetailAccount}>
              🔄 تلاش مجدد
            </Button>
          </div>
        </Alert>
  ...
  return (
    <Container>
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          <h1 className="mb-1">جزئیات حساب تفصیلی</h1>
        ...
        <div className="d-flex gap-2">
          ...
          <Button 
            ...
            onClick={() => router.push(`/accounts/${account.subAccountId}`)}
          >
            🔗 مشاهده حساب معین
          </Button>
        </div>
        ...
        <Col md={4}>
          <Card className="mb-4">
            <Card.Header className="d-flex justify-content-between align-items-center">
              <h5 className="mb-0">📊 اطلاعات حساب تفصیلی</h5>
              <Badge bg={getTypeColor(accountType)}>
            ...
            <Card.Body>
              <table className="table table-borderless table-sm">
                <tbody>
                  <tr>
                    <td className="fw-bold text-muted" width="140">کد حساب تفصیلی:</td>
                    <td className="fw-bold h6 font-monospace">{account.code}</td>
                  </tr>
                  <tr>
                  ...
                  <tr>
                    ...
                    <td>
                      {account.subAccount.category.code} - {account.subAccount.category.name}
                    </td>
                  </tr>
                  <tr>
          ...
          <Card className="bg-light">
            <Card.Header>
              <h6 className="mb-0">📈 آمار حساب تفصیلی</h6>
            </Card.Header>
            <Card.Body>
        ...
        <Col md={8}>
          <Card>
            <Card.Body>
              ...
              >
                {/* تب تراکنش‌ها */}
                <Tab eventKey="transactions" title="🔄 تراکنش‌ها">
                  <div className="d-flex justify-content-between align-items-center mb-3">
                    <h6 className="mb-0">تراکنش‌های حساب تفصیلی</h6>
                    <Badge bg="info">
                  ...
                  {transactionCount > 0 ? (
                    ...
                    <div className="text-center py-4">
                      ...
                      <p className="text-muted">
                        این حساب تفصیلی تاکنون در هیچ سند حسابداری استفاده نشده است.
                      </p>
                      <Button 
                        variant="outline-primary"
                ...
                <Tab eventKey="monthly" title="📅 گردش ماهانه">
                  <h6 className="mb-3">گردش حساب تفصیلی بر اساس ماه</h6>
                    ...
                    <Table striped bordered hover size="sm">
                      ...
                      <tbody>
                        {monthlyTurnover.map((monthData, index) => {
                          const net = accountType === 'asset' || accountType === 'expense' 
                    ...
                    <Col md={6}>
                      ...
                      <table className="table table-borderless table-sm">
                        <tbody>
                          ...
                          <tr>
                            <td className="fw-bold text-muted">شناسه حساب معین:</td>
                            <td>{account.subAccountId}</td>
                          </tr>
                          <tr>
                            <td className="fw-bold text-muted">کد حساب معین:</td>
                            <td>{account.subAccount.code}</td>
                          </tr>
                          <tr>
                            <td className="fw-bold text-muted">کد حساب کل:</td>
                            <td>{account.subAccount.category.code}</td>
                          </tr>
                          <tr>
                    ...
                    <Col md={6}>
                      ...
                      <table className="table table-borderless table-sm">
                        <tbody>
                          ...
                          <tr>
                            <td className="fw-bold text-muted">موجودی در دیتابیس:</td>
                            <td className="fw-bold">{formatCurrency(account.balance)}</td>
                          </tr>
                        </tbody>
                      </table>
                    </Col>
